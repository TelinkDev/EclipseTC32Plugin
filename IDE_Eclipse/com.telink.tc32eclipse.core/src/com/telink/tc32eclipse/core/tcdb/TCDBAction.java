 ...
 * 
 */
public class TCDBAction {

	public enum MemType {
		// The names of the is currently just speculation because TCDB does not
	...
	public TCDBAction(MemType memtype, Action action, String filename, FileType filetype) {
	...
	public TCDBAction(MemType memtype, Action action, int value) {
		fMemType = memtype;
		fAction = action;
	...
	/**
	 * Get the TCDB action option without resolving the filename.
	 * 
	 * @return <code>String</code> with an TCDB option
	 */
	...
	public String getArgument(IConfiguration buildcfg) {
		...
		if (fFileType.equals(FileType.immediate)) {
			...
			if (buildcfg != null) {
				...
				// PS filename = new Path(resolvedfilename).toOSString();
				filename = new Path(resolvedfilename).toPortableString();
			} else {
				filename = fFilename;
			}
	 ...
	 * information required to make a new <code>TCDBAction</code>
	 * </p>
	 * <p>
	 ...
	 * @return
	 */
	public static TCDBAction getActionForArgument(String argument) {
		...
		// TCDBProperties keeps the Actions to itself. And changing
		// TCDBProperties would have broken more stuff, so I use this little
		...
		// TCDB, although this is more forgiving.

		// The four parts of info we extract from the argument
		// FileType is optional for TCDB and defaults to "auto"
		MemType memtype;
		Action action;
		String filename;
		...
		if (!matcher.matches())
			...
			throw new IllegalArgumentException("Invalid argument for TCDB");
		...
		if (p == -1) {
			...
			return new TCDBAction(MemType.flash, Action.write, arg, FileType.auto);
		}
		...
		if (filetype == FileType.immediate) {
			result = new TCDBAction(memtype, action, Integer.decode(filename));
		} else {
			result = new TCDBAction(memtype, action, filename, filetype);
		}
