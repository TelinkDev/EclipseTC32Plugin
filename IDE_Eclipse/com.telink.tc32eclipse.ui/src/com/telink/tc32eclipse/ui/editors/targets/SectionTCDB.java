 ...
 * 
 */
public class SectionTCDB extends AbstractTCSectionPart {
	...
	private final static String[]	PART_ATTRS		= new String[] { TCDBTool.ATTR_CMD_NAME,
			TCDBTool.ATTR_USE_CONSOLE, TCDBTool.ATTR_VERBOSITY };
	...
	protected void refreshSectionContent() {
		String command = getTargetConfiguration().getAttribute(TCDBTool.ATTR_CMD_NAME);
		fCommandText.setText(command);
		validateCommand();

		int verbosity = getTargetConfiguration().getIntegerAttribute(TCDBTool.ATTR_VERBOSITY);
	...
	private void addCommandText(Composite parent, FormToolkit toolkit) {
		...
		fCommandText
				.setToolTipText("The command to start TCDB.\nIf the TCDB executable is not on the system path,\nthen the full path to the executable must be provided.");

		fCommandText.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				String command = fCommandText.getText();
				getTargetConfiguration().setAttribute(TCDBTool.ATTR_CMD_NAME, command);
		...
		//
		fVersionLabel = toolkit.createLabel(parent, "TCDB version");
	...
	private void addVerbosityCombo(Composite parent, FormToolkit toolkit) {
		...
		fVerbosityCombo.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				getTargetConfiguration().setIntegerAttribute(TCDBTool.ATTR_VERBOSITY, newindex);
				getManagedForm().dirtyStateChanged();
			}
		});
	...
	private void addAdvancedSection(Composite parent, FormToolkit toolkit) {
		...
		section.setText("Advanced TCDB settings");
	 ...
	 * <p>
	 * Clicking the Button will search the file system for TCDB and the result is copied to the
	...
	protected Button createSearchButton(FormToolkit toolkit, Composite parent, final Text text) {
		...
		button.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent event) {
				// TODO: Implement search for TCDB
			}
		});
		return button;
	}
	...
	private void validateCommand() {
		...
		validate(TCDBTool.ATTR_CMD_NAME, fCommandText, fCVL);
	}

	private final CommandValidationListener	fCVL	= new SectionTCDB.CommandValidationListener();
