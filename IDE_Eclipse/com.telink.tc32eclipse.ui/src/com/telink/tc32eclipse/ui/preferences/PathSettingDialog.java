 ...
 * Purpose:
 * Reference :   
 * $Id: PathSettingDialog.java 851 21.0.38-07 19:37:00Z innot $
 *     
 *******************************************************************************/
...
import com.telink.tc32eclipse.core.paths.TC32PathManager.SourceType;
...
public class PathSettingDialog extends StatusDialog {
	...
	private TC32PathManager	fPathManager	= null;
	 ...
	 */
	public PathSettingDialog(Shell parent, TC32PathManager pathmanager) {
		super(parent);

		// make a copy of the given IPathManager
		fPathManager = new TC32PathManager(pathmanager);
	...
	protected Control createDialogArea(Composite parent) {
		...
		String[] types = { TC32PathManager.SourceType.System.toString(),
				TC32PathManager.SourceType.Bundled.toString(),
				TC32PathManager.SourceType.Custom.toString() };
	...
	private Composite addBundlePage(Composite parent) {
		...
		Label label = new Label(page, SWT.NONE);
		label.setText("Select tc32-elf-gcc Bundle");
	 ...
	 */
	public TC32PathManager getResult() {
		return fPathManager;
	}
	...
	private void changeSourceType(String type) {
		...
		if (type.equals(SourceType.Bundled.toString())) {
			...
			updateStatus(new Status(IStatus.ERROR, TC32Plugin.PLUGIN_ID,
	...
	private void testStatus() {
		...
		if (empty && valid) {
			// warning only
			status = new Status(IStatus.WARNING, TC32Plugin.PLUGIN_ID, "Optional path is empty");
		}
		if (!valid) {
			// error
			status = new Status(IStatus.ERROR, TC32Plugin.PLUGIN_ID, "Path is invalid");
		}
