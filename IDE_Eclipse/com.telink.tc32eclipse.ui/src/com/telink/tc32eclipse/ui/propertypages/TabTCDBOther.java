...
/**
* The TCDB Other Actions Tab page.
* <p>
...
* <li>The "other options" field for TCDB options not covered by the plugin</li>
* </ul>
* </p>
* 
* @author Peter Shieh
...
* 
*/
public class TabTCDBOther extends AbstractTCDBPropertyTab {

	// The GUI texts

	// Flash erase cycle counter check
	private final static String	GROUP_COUNTER			= "Flash memory erase cycle counter (-y)";
	private final static String	LABEL_COUNTER			= "Enable this to have TCDB count the number of flash erase cycles.\n"
	...
	private void readCycleCounter() {
		...
		readJob.setRule(new TCDBSchedulingRule(fTargetProps.getProgrammer()));
	...
	private void writeCycleCounter() {
		...
		// 65534 is the largest value accepted (65535 (0xffff) is interpreted by TCDB as an unset
		// cycle counter).
		if (newvalue >= 0xfffe) {
			newvalue = 0xfffe;
		}
	...
	private class CycleJob extends Job {
		...
		protected IStatus run(IProgressMonitor monitor) {
			...
			try {
				...
				if (fWrite) {
					loadedvalue = TCDB.getDefault().setEraseCycleCounter(
							fTargetProps.getProgrammer(), fNewCounterValue,
							new SubProgressMonitor(monitor, 95));
				} else {
					loadedvalue = TCDB.getDefault().getEraseCycleCounter(
				...
				if (!fButton.isDisposed()) {
					UIJob messagejob = new TCDBErrorDialogJob(fButton.getDisplay(), ade,
